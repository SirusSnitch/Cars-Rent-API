...
DATABASE_URL = "postgresql+asyncpg://postgres:postgres@db_rent:5432/rental_service"
...
async def rent_car(user_id: int, car_id: int, db: AsyncSession = Depends(get_db)):
    ...
    async with httpx.AsyncClient() as client:
        car_response = await client.get(f"{CAR_SERVICE_URL}/cars/{car_id}")
...
async def list_rentals(db: AsyncSession = Depends(get_db)):
    ...
    async with httpx.AsyncClient() as client:
        for rental in rentals:
            # Fetch user
            user_response = await client.get(f"{USER_SERVICE_URL}/users/{rental.user_id}")
            if user_response.status_code == 200:
            ...
            # Fetch car
            car_response = await client.get(f"{CAR_SERVICE_URL}/cars/{rental.car_id}")
